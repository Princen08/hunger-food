...
def test_create_order_exception(db_mock):
    ...
    db_mock.table.return_value.insert.return_value.execute.side_effect = Exception(
        "DB Error"
    )
...
def test_get_orders_exception(db_mock):
    db_mock.table.return_value.select.return_value.execute.side_effect = Exception(
        "DB Error"
    )
...
def test_get_order_success(db_mock):
    ...
    db_mock.table.return_value.select.return_value.eq.return_value.execute.return_value = (
        mock_response
    )
...
def test_get_order_exception(db_mock):
    db_mock.table.return_value.select.return_value.eq.return_value.execute.side_effect = Exception(
        "DB Error"
    )
...
def test_delete_order_success(db_mock):
    ...
    db_mock.table.return_value.delete.return_value.eq.return_value.execute.return_value = (
...
def test_delete_order_not_found(db_mock):
    ...
    mock_delete_response.data = (
        []
    )  # Simulate no data returned after delete (order not found)
    db_mock.table.return_value.delete.return_value.eq.return_value.execute.return_value = (
        mock_delete_response
    )
    ...
    assert (
        deleted_order_response is None
    )  # crud.delete_order returns None if data is empty
    db_mock.table.assert_called_with("orders")
    db_mock.table().delete.assert_called_once()
...
def test_delete_order_exception_on_select(db_mock):
    ...
    db_mock.table.return_value.delete.return_value.eq.return_value.execute.side_effect = Exception(
        "DB Error on Delete"
    )
