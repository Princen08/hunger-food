...
patch_create_client = patch(
    "supabase.create_client", return_value=mock_supabase_instance
)
patch_create_client.start()

# 2. Patch other utilities (auth, logger)
# Patch authenticate_user
patch_authenticate_user = patch(
    "app.utils.auth.authenticate_user", return_value="test_user_id"
)
